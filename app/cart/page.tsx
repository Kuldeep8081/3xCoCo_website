...
function CartContent() {
  ...
  // FIX: Only auto-select items ONCE when they first load.
  // This prevents re-selecting everything when you change quantity.
  useEffect(() => {
  ...
  const toggleAll = () => {
    if (selectedIds.size === items.length) {
      ...
      setSelectedIds(new Set(items.map((i) => i._id))); // Check all
    }
  };
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          console.error("Geocoding error:", error); // FIX: Log the error
          alert("Failed to get address.");
        } finally {
          setLocating(false);
        }
      },
      (error) => { // This error was already used correctly
      ...
      const res = await fetch("/api/checkout", {
        ...
        body: JSON.stringify({
          ...
          customerDetails: {
            ...
            pricingBreakup: { baseAmount, gstAmount, total, discount, shipping, payableTotal },
          },
        }),
      });
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              <div className="flex items-center gap-3 pb-2 border-b border-[#3d2326] mb-2 px-2">
                <button 
                  onClick={toggleAll}
                  className="flex items-center gap-2 text-sm font-medium text-[#FFB368] hover:text-[#FFECDC] transition"
                >
                  {isAllSelected ? (
                    <CheckSquare size={18} className="text-[#FFB368]" />
                  ) : (
                    <Square size={18} className="text-[#573032]" />
                  )}
                  {isAllSelected ? "Deselect All" : "Select All"}
                </button>
                <span className="text-xs text-[#c8a898] ml-auto">
